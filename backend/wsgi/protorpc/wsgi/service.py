    ...
    def protorpc_service_app(environ, start_response):
        ...
        def send_rpc_error(status_code, state, message, error_name=None):
            ...
            Returns:
              ...
              the request.
            """
            status = remote.RpcStatus(
                state=state, error_message=message, error_name=error_name
            )
        ...
        initialize_request_state = getattr(instance, "initialize_request_state", None)
        ...
        try:
            ...
            logging.exception(
                "Encountered unexpected error from ProtoRPC "
                "method implementation: %s (%s)" % (err.__class__.__name__, err)
            )
            return send_rpc_error(
