    ...
    def request_type(self):
        ...
        if isinstance(self.__request_type, str):
            self.__request_type = messages.find_definition(
                self.__request_type, relative_to=sys.modules[self.__method.__module__]
            )
    ...
    def response_type(self):
        ...
        if isinstance(self.__response_type, str):
            self.__response_type = messages.find_definition(
                self.__response_type, relative_to=sys.modules[self.__method.__module__]
            )
        return self.__response_type


def method(
    request_type=message_types.VoidMessage, response_type=message_types.VoidMessage
):
    ...
    if not isinstance(request_type, str) and (
        ...
        or not issubclass(request_type, messages.Message)
        or request_type is messages.Message
    ):
        raise TypeError(
            "Must provide message class for request-type.  Found %s", request_type
        )

    if not isinstance(response_type, str) and (
        not isinstance(response_type, type)
        or not issubclass(response_type, messages.Message)
        or response_type is messages.Message
    ):
        raise TypeError(
            "Must provide message class for response-type.  Found %s", response_type
        )
    ...
    def remote_method_wrapper(method):
        ...
        def invoke_remote_method(service_instance, request):
            ...
            if not isinstance(request, remote_method_info.request_type):
                raise RequestError(
                    "Method %s.%s expected request type %s, "
                    "received %s"
                    % (
            ...
            if not isinstance(response, remote_method_info.response_type):
                raise ServerError(
                    "Method %s.%s expected response type %s, "
                    "sent %s"
                    % (
                        type(service_instance).__name__,
                        method.__name__,
...
def remote(request_type, response_type):
    ...
    logging.warning(
        "The remote decorator has been renamed method.  It will be "
...
class _ServiceClass(type):
    ...
    def __init__(cls, name, bases, dct):
        ...
        if StubBase not in bases:
            ...
            stub_attributes = {"Service": cls, "__init__": __init__}
...
class RequestState(object):
    ...
    def _repr_items(self):
        for name in ["remote_host", "remote_address", "server_host", "server_port"]:
...
class ProtocolConfig(object):
    ...
    def __init__(
        self, protocol, name, default_content_type=None, alternative_content_types=None
    ):
        """Constructor.
        ...
        self.__name = name
        self.__default_content_type = (
            default_content_type or protocol.CONTENT_TYPE
        ).lower()
        if alternative_content_types is None:
            alternative_content_types = getattr(
                protocol, "ALTERNATIVE_CONTENT_TYPES", ()
            )
        ...
        self.__content_types = (
            self.__default_content_type,
        ) + self.__alternative_content_types
