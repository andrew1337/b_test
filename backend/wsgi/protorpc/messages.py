            ...
            try:
                ...
                try:
                    ...
                    raise InvalidDefaultError(
                        "Invalid default value for %s: %s: %s"
            ...
            if value is None:
                ...
                try:
                    ...
                    raise ValidationError(
                        ...
                        "found %s (type %s)" % (self.type, name, value, type(value))
                    )
            ...
            try:
                ...
                try:
                    ...
                    validation_error = ValidationError(
                        "Field encountered non-ASCII string %s: %s" % (value, err)
                    )
                else:
                    validation_error = ValidationError(
                        "Field %s encountered non-ASCII string %s: %s"
                        % (self.name, value, err)
                    )
        ...
        if isinstance(message_type, type) and issubclass(message_type, Message):
            if self.repeated:
                if value and isinstance(value, (list, tuple)):
                    value = [
                        (message_type(**v) if isinstance(v, dict) else v) for v in value
                    ]
...
def find_definition(name, relative_to=None, importer=__import__):
    ...
    def search_path():
        ...
        for node in name_path:
            ...
            if not isinstance(next, types.ModuleType) and not (
                isinstance(next, type) and issubclass(next, (Message, Enum))
            ):
                return None
